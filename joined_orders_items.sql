...
{% if is_incremental() %}
    ...
    WHERE o.ordered_at >= (COALESCE((SELECT MAX(ordered_at) - INTERVAL '2 DAYS' FROM {{ this }}), 
                        '2016-09-01'::timestamp) - INTERVAL '2 DAY')
    AND o.ordered_at < (COALESCE((SELECT MAX(ordered_at) + INTERVAL '2 DAYS' FROM {{ this }}), 
                        '2016-09-01'::timestamp) + INTERVAL '2 DAY')
{% endif %}
